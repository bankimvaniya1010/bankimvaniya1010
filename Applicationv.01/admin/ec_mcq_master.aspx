<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ec_mcq_master.aspx.cs" Inherits="admin_ec_mcq_master" MasterPageFile="~/admin/admin.master"%>
...
<asp:Content ID="content2" runat="server" ContentPlaceHolderID="ContentPlaceHolder1">
    <div class="container page__container">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">MCQ Master</li>
        </ol>
        <h1 class="h2">MULTIPLE CHOICE QUESTION (MCQ) MASTER</h1>
            ...
            <div class="media align-items-center">
                <div class="form-row">
                    <table>
                        <tr>
                            <td>
                                <asp:Button ID="btn_addnew" runat="server" Text="Create a MCQ" CssClass="btn btn-success" OnClick="btn_addnew_Click" />
                                <br />
                            </td>
                        </tr>
                    ...
                    <asp:GridView ID="QuestiontGridView" CssClass="table" runat="server" AutoGenerateColumns="False"
                        ...
                        <Columns>
                            ...
                            <asp:TemplateField HeaderText="View Question">
                                <ItemTemplate>
                                    <a onclick="showModelQuestion('<%# Eval("questionname")%>','<%# Eval("question")%>','<%# Eval("answer1")%>','<%# Eval("answer2")%>','<%# Eval("answer3")%>','<%# Eval("answer4")%>','<%# Eval("answer_description1")%>','<%# Eval("answer_description2")%>','<%# Eval("answer_description3")%>','<%# Eval("answer_description4")%>','<%# Eval("marks")%>','<%# Eval("time")%>','<%# Eval("questionfor")%>','<%# Eval("correctanswer")%>')" style="color: rgba(33,150,243,.85);text-decoration: underline;">
                                        View
                                    </a>
                                </ItemTemplate>
                            </asp:TemplateField>
