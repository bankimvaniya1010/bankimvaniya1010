...
public partial class gte_certificateN_IN : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        var scheduledate = db.applicant_meeting_schedule.Where(x => x.applicant_id == applicantID && x.university_id == universityID && x.is_meetingtime_expires == null).FirstOrDefault();
                    ...
                    if (Utility.CheckStudentLogin())
                    {
                        ...
                        if (!IsPostBack)
                        {
                            ...
                            if (studentGteProgress != null)
                            {
                                ...
                                {                                    
                                    if (studentGteProgress.is_gte_certificate_generated != null && studentGteProgress.is_gte_certificate_generated.HasValue && studentGteProgress.is_gte_certificate_generated.Value)
                                    {
                                        setStudentPersonalDetails(ApplicantID, universityID);
                                    }
                                    else
                                    {
                                        ...
                                        int userPercentageScore = (int)Math.Round((decimal)userScore / totalQuestion * 100);//(int)Math.Ceiling((decimal)userScore / totalQuestion * 100);
                                        if (90 <= userPercentageScore && userPercentageScore <= 100)
                                            generateParticipationCertificate("1", userPercentageScore);//generateParticipationCertificate(GetGrade(Convert.ToInt32(1)));
                                        else if (80 <= userPercentageScore && userPercentageScore <= 89)
                                            generateParticipationCertificate("2", userPercentageScore);
                                        else if (70 <= userPercentageScore && userPercentageScore <= 79)
                                            generateParticipationCertificate("3", userPercentageScore);
                                        else if (60 <= userPercentageScore && userPercentageScore <= 69)
                                            generateParticipationCertificate("4", userPercentageScore);
                                        else if (50 <= userPercentageScore && userPercentageScore <= 59)
                                            generateParticipationCertificate("5", userPercentageScore);
                                        else if (40 <= userPercentageScore && userPercentageScore <= 49)
                                            generateParticipationCertificate("6", userPercentageScore);
                                        else if (0 <= userPercentageScore && userPercentageScore <= 39)
                                            generateParticipationCertificate("7", userPercentageScore);
    ...
    private void generateParticipationCertificate(string performanceCategor, int percentage)
    {
        try
        {
            ...
            if (gteProgressBar != null)
            {
                ...
                gteProgressBar.performance_category = performanceCategor;
                gteProgressBar.gte_perdentage_score = percentage.ToString()+"%";
    ...
    protected void btnDownload_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            bool? Is_new_applicant = objCom.GetIS_oldOrNew_applicant(applicantID);
    ...
    private void setStudentPersonalDetails(int applicantId, int universityID)
    {
        try
        {
            ...
            if (Is_new_applicant == true)
                DivCertificate.Attributes.Add("style", "display:block");
            else
            {
            ...
            if (applicant_gte_progress != null)
            {
                ...
                if (applicant_gte_progress.certificate_creation_date.HasValue)
                    _certificateCreationDate = applicant_gte_progress.certificate_creation_date.Value.ToString("dd-MMM-yyyy");
                ...
                QRURL = webURL + "gte_certificateN.aspx?CID=" + applicant_gte_progress.certificate_name;
            }
    ...
    private void showContentOLd(int aid)
    {
        try
        {
            if (downloadPdf == 1 && Request.QueryString["token"] != null && Request.QueryString["token"].ToString() == "XS7MKjHLunMAvqzCGr")
            ...
            else if (downloadPdf == 0)
            {
                ...
                if (Utility.CheckStudentLogin())
                {
                   ...
                   // ApplicantID = Convert.ToInt32(Session["UserID"].ToString());
                    ...
                    if (!IsPostBack)
                    {
                        var studentGteProgress = db.gte_progressbar.Where(x => x.applicantid == aid && x.universityId == universityID).FirstOrDefault();
                        if (studentGteProgress != null)
                        {
                            if (studentGteProgress.is_gte_preliminarysection2_completed != null)
                            {
